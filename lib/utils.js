...
function isIn(str, table) {
    for (let i = 0; i < str.length; i++) {
        const codeAscii = str.charCodeAt(i);
        if (!table[codeAscii]) {
            return false;
        }
    }
...
function checkUsernameEmail(username, acceptPlus) {
    ...
    for (const item of usernameItems) {
        ...
        if (first === false || last === false || middle === false) {
            ...
            ret.info = "Invalid username encoding";
            return (ret);
        }
 ...
 * Uses queueMicrotask when available (modern browsers & Node.js),
 * falls back to setTimeout for older environments.
